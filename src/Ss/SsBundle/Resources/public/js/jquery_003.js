		...
		if(cart.item_count > 0) {
			...
			jQ.each(cart.items, function(index, cartItem) {
				...
				if (index == (cart.items.length-1)) {
					$("#drawer .spinner").remove();
					$("#drawer .container").prepend('<form action="/cart" method="post">'+drawer+'<div class="actions"><input type="submit" onclick="window.location.href=\'/cart\';return false;" value="Check Out" /></div></form>').hide().fadeIn(500);
				}
			});
